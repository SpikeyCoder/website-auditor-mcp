...
export const P0_TOOLS: ToolSpec[] = [
  ...
  },
  {
    ...
    description:
      'Report what changed in a website\'s AI visibility and audit since it was last checked. Use this when someone asks "did anything change," "what\'s different this week/month," "did my AI visibility drop," or "did a competitor overtake me." Requires the domain to be tracked (see track_site). Returns deltas: score movement, engines gained/lost, competitors that moved, and new or resolved issues. A change is only ever measured between two snapshots that asked the same question (the same business name, market and queries); when there is no such pair it says why instead of giving a number, with the date the series re-baselined when it did. The series is the weekly re-audits, and any audit that asked their question, while the newest re-audit is within four weeks and a day of the newest snapshot; otherwise every snapshot.',
    inputSchema: {
      domain: domainArg,
