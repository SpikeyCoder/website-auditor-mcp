...
export const MONITORING_TOOLS: ToolSpec[] = [
  ...
  },
  {
    ...
    description:
      'Get a glanceable summary of monitoring status across all tracked websites. Use this when someone asks "how are my tracked sites doing," "what\'s my current AI visibility across everything I monitor," "when were my sites last checked or when do they run next," or wants a dashboard of their monitored domains. Returns, per domain, the latest AI-visibility score of its series (the measured weekly re-audits and any audit that asked their question, while it has no gap longer than four weeks and a day from the newest weekly re-audit that recorded its question to the newest measured snapshot; otherwise every measured snapshot; a newer snapshot outside the series is not shown, and get_changes names it), when it was last audited and next runs, and the most recent change against the last snapshot that asked the same question, or a note saying why there is none yet.',
    inputSchema: {},
  },
];
